...
namespace Usuario
{
    public partial class WebForm2 : System.Web.UI.Page
    {
        ...
        protected void btFiltCant_Click(object sender, EventArgs e)
        {
            ...
            {
                Response.Redirect("WebForm2.aspx?cantidad=" + cantidad); // Redirige con el valor de cantidad
            }  
            else if (cantidad == "") // Si no se coloca valor
            {
                Response.Redirect("WebForm2.aspx"); // Redirige con todos los datos
            }
            else
            {
        ...
        protected void btFiltNombre_Click(object sender, EventArgs e)
        {
            ...
            if (nombre.Length > 0)
                Response.Redirect("WebForm2.aspx?nombre=" + nombre); // Si no esta vacia se filtra
            else
                Response.Redirect("WebForm2.aspx"); // Si esta vacia se despliega todo
        }

        protected void btFiltClase_Click(object sender, EventArgs e)
        {
            Response.Redirect("WebForm2.aspx?clase=" + DdClaseList.SelectedValue); // Filtra por la opcion de la caja
        }

        protected void btInsertar_Click(object sender, EventArgs e)
        {
            Response.Redirect("InsertarArtic.aspx"); // Me redirige a la pagina de insertar
        }

        protected void btSalir_Click(object sender, EventArgs e)
        {
            Response.Redirect("WebForm1.aspx"); // Me redirige a la pagina de usuario
        }
    }
}